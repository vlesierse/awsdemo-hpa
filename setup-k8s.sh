...
aws iam create-policy   \
  ...
  --policy-document file://k8s/cluster-autoscaler-policy.json
...
aws iam create-policy   \
  ...
  --policy-document file://k8s/k8s-cloudwatch-adapter-policy.json
...
eksctl create iamserviceaccount \
    ...
    --approve \
    --override-existing-serviceaccounts
kubectl apply -f ./k8s/k8s-cloudwatch-adapter.yaml
